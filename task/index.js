...
function run() {
    return __awaiter(this, void 0, void 0, function () {
        var inputString, delimitedString;
        return __generator(this, function (_a) {
            try {
                inputString = task.getInput('envKvp', true);
                delimitedString = inputString === null || inputString === void 0 ? void 0 : inputString.split(',');
                if (delimitedString && delimitedString.length > 0) {
                    delimitedString.forEach(function (str) {
                        var kvp = str.split('=');
                        var key = kvp[0].trim();
                        var val = kvp[1].trim();
                        task.debug("Set $(" + key + ") with '" + val + "'");
